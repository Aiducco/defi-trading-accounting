...
_LOG_PREFIX = "[TRADE-REPORT-VIEW]"


class TradeReport(views.View):
    def get(
        self, request: http.HttpRequest, *args: typing.Any, **kwargs: typing.Any
    ) -> http.HttpResponse:
        logger.info("{} Generating trade history report.".format(_LOG_PREFIX))
        try:
            validated_query_params = common_utils.validate_data_schema(
                data=request.GET, schema=api_schemas.TradeExportQuerySchema()
            )
        ...
        logger.info(
            "{} Generating trade history report (wallet_address={}, provider={}, from_datetime={}, to_datetime={}).".format(
                _LOG_PREFIX,
        ...
        try:
            report = trading_provider_services.get_trade_history_report(
            ...
            logger.exception(
                "{} Unexpected exception while generating trade history report (wallet_address={}, provider={}, from_datetime={}, to_datetime={}). Error: {}.".format(
                    _LOG_PREFIX,
                    validated_query_params["wallet_address"],
                    validated_query_params["provider"],
                    from_date,
        ...
        return http.HttpResponse(
            headers={
                ...
                "Content-Disposition": "attachment; filename=trade_history_{}_{}_{}_{}.csv".format(
                    validated_query_params["provider"],
                    validated_query_params["wallet_address"],
                    from_date,
                    to_date,
            ...
            content=common_utils.get_csv_file_writer(
                headers=[
                    field.name
                    for field in dataclasses.fields(provider_messages.OrderHistory)
