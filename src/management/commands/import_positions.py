...
class Command(BaseCommand):
    help = """
            Imports all positions for all wallet addresses in accounting_wallet.
            ex. python manage.py import_positions --provider=HYPERLIQUID
            """

    log_prefix = "[IMPORT-POSITIONS]"
    ...
    def handle(self, *args: typing.Any, **kwargs: typing.Any) -> None:
        ...
        if not accounting_wallets:
            logger.info(
                "{} No accounting wallets to import positions for. Exiting.".format(
                    self.log_prefix
                )
            )
            return
        ...
        logger.info(
            "{} Found {} accounting wallets to import positions for.".format(
        ...
        for accounting_wallet in accounting_wallets:
            logger.info(
                "{} Importing positions data (wallet_address={}, accounting_wallet_id={}).".format(
            ...
            try:
                ...
                logger.exception(
                    "{} Unexpected exception occurred while importing positions data. Error: {}.".format(
                        self.log_prefix,
