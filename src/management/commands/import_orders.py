...
class Command(BaseCommand):
    help = """
            Imports all orders (FILLED AND OPEN) for all wallet addresses in accounting_wallet.
            ex. python manage.py import_orders --provider=HYPERLIQUID
            """

    log_prefix = "[IMPORT-TRADE-ORDERS]"
    ...
    def handle(self, *args: typing.Any, **kwargs: typing.Any) -> None:
        ...
        if not accounting_wallets:
            logger.info(
                "{} No accounting wallets to import trade order for. Exiting.".format(
                    self.log_prefix
                )
            )
            return
        ...
        logger.info(
            "{} Found {} accounting wallets to import trade orders for.".format(
        ...
        for accounting_wallet in accounting_wallets:
            logger.info(
                "{} Importing trade order data (wallet_address={}, accounting_wallet_id={}).".format(
                    self.log_prefix, accounting_wallet.address, accounting_wallet.id
                )
            )
            try:
                trading_data_service_client = trading_data_importer.TradingDataImporter(
                    trading_data_provider=trading_provider_factory.Factory.create(
                        provider=trading_provider
                    )
                )
                trading_data_service_client.import_open_trade_orders(
                    wallet_address=accounting_wallet.address
                )
                trading_data_service_client.import_trade_orders(
                    wallet_address=accounting_wallet.address,
                ...
                logger.exception(
                    "{} Unexpected exception occurred while importing trade order data. Error: {}.".format(
                        self.log_prefix,
